    ...
    list.innerHTML += `
      <div class="series-card">
        <div class="score-display">
          <div style="flex:1"><strong>${tA}</strong><div class="score-num">${wA}</div></div>
          <div style="flex:1; text-align:right;"><strong>${tB}</strong><div class="score-num">${wB}</div></div>
        </div>
        <div class="p-bar-bg">
        ...
        <div class="btn-group" style="display:flex; gap:8px; margin-top:10px;">
          <button class="btn btn-primary" style="flex:2" onclick="addWin('${s.id}', '${tA}', '${tB}')">Win: ${tA.split(' ')[0]}</button>
          <button class="btn btn-primary" style="flex:2; background:var(--gold)" onclick="addWin('${s.id}', '${tB}', '${tA}')">Win: ${tB.split(' ')[0]}</button>
          <button class="btn btn-end" onclick="endSeriesEarly('${s.id}')">STOP</button>
        </div>
      </div>
    `;
  });
...
window.endSeriesEarly = async (sId) => {
    if(confirm("End this series now? It will be saved as a 'Random Game' in your history.")) {
        await addDoc(gamesRef, {
            seriesId: sId, manualEnd: true, time: new Date().toISOString(), secret: SECRET_KEY
        });
    }
};
...
window.addWin = async (sId, winner, loser) => {
  ...
  await addDoc(gamesRef, {
    seriesId: sId, winner, loser, seriesTarget: s.target,
    time: new Date().toISOString(), secret: SECRET_KEY
  });
};
...
window.startNewSeries = () => {
  ...
  showModal(`
    <div class="confirm-box">
      <h3 style="margin-top:0">Start Series</h3>
      <select id="mTarget" class="btn btn-secondary" style="width:100%; margin-bottom:10px;"><option value="5">First to 5</option><option value="10">First to 10</option></select>
      <select id="mTA" class="btn btn-secondary" style="width:100%; margin-bottom:10px;">${opts}</select>
      <select id="mTB" class="btn btn-secondary" style="width:100%; margin-bottom:10px;">${opts}</select>
      <button class="btn btn-primary" style="width:100%; margin-bottom:8px" onclick="confirmStart()">Start Now</button>
...
window.confirmStart = async () => {
    ...
    await addDoc(gamesRef, {
        seriesId: sId, winner: tA, loser: tB, time: new Date().toISOString(),
...
function updateStatsPage() {
    ...
    let html = `<div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:20px;">
        <div class="card" style="margin:0; text-align:center; padding:10px;"><small>Individual Wins</small><div style="font-size:18px; font-weight:800">${gameData.games.filter(g=>!g.manualEnd).length}</div></div>
        <div class="card" style="margin:0; text-align:center; padding:10px;"><small>Competitive Series</small><div style="font-size:18px; font-weight:800">${finished.filter(s=>!s.isRandom).length}</div></div>
    </div>`;

    html += `<h3 style="font-size: 14px; color: var(--gold);">Global Rankings</h3>`;
    Object.entries(teamStats).sort((a,b) => b[1].wins - a[1].wins).forEach(([name, data]) => {
        html += `<div style="display:flex; justify-content:space-between; padding:10px 0; border-bottom:1px solid rgba(255,255,255,0.05); font-size:13px;">
            <span>${name}</span>
            <span><strong>${data.wins}W</strong> - ${data.losses}L</span>
        </div>`;
    });

    html += `<h3 style="font-size: 14px; color: var(--gold); margin-top:25px;">Series History</h3>`;
    finished.slice().reverse().forEach(s => {
        ...
        const teams = Object.keys(s.wins);
        const scoreStr = teams.length > 1 ? `${s.wins[teams[0]]} - ${s.wins[teams[1]]}` : "Game Ended Early";
        html += `<div style="font-size:12px; margin-bottom:12px; background: rgba(255,255,255,0.02); padding: 10px; border-radius: 8px;">
